    ...
    **kwargs,
    ...
    for epoch in range(num_epoch):
        ...
        for img, label in train_data:
            ...
            loss.backward()
            optimizer.step()
            # metrics["train_acc"].append((output.argmax(dim=1) == label).float().mean().item())
            metrics["train_acc"].append(compute_accuracy(output, label))
        ...
        with torch.inference_mode():
            ...
            for img, label in val_data:
                ...
                output = model(img)
                metrics["val_acc"].append(compute_accuracy(output, label))
        ...
        epoch_train_acc = sum(metrics["train_acc"]) / len(metrics["train_acc"])
        epoch_val_acc = sum(metrics["val_acc"]) / len(metrics["val_acc"])
