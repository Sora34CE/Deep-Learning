...
"""
DISCLAIMER: Some of my code was suggested with Google Colab's Gemini AI.
"""
...
class WeatherForecast:
    ...
    def find_min_and_max_per_day(self) -> Tuple[torch.Tensor]:
        ...
        """
        return torch.as_tensor(self.data.min(dim=1).values), torch.as_tensor(self.data.max(dim=1).values)
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        indexes = torch.max(torch.abs(self.data - torch.mean(self.data * 1.0, dim=1).unsqueeze(dim=1)), dim=1)[1]
        return torch.gather(self.data, dim=1, index=indexes.unsqueeze(dim=1)).squeeze(dim=1)
    ...
    def predict_temperature(self, k: int) -> torch.Tensor:
        ...
        """
        return (torch.mean(self.data[-k:])).float()
